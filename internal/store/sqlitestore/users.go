...
func (s *SQLiteUserStore) Create(ctx context.Context, user *store.UserData) error {
	...
	_, err := s.db.ExecContext(ctx,
		...
		user.Status, user.Phone, user.Timezone, user.LastLoginAt, user.CreatedAt, user.UpdatedAt,
	)
	return err
}

const userCols = `id, email, display_name, avatar_url, auth_provider, password_hash, status, phone, timezone, last_login_at, created_at, updated_at`

func scanUser(row interface{ Scan(dest ...any) error }) (*store.UserData, error) {
	var u store.UserData
	var id string
	var avatarURL, passwordHash, phone, timezone *string
	...
	err := row.Scan(
		...
		&u.Status, &phone, &timezone, &lastLoginAt, createdAt, updatedAt,
	)
	if err != nil {
		return nil, err
	}
...
func (s *SQLiteUserStore) Update(ctx context.Context, user *store.UserData) error {
	...
	res, err := s.db.ExecContext(ctx,
		`UPDATE users SET display_name = ?, avatar_url = ?, phone = ?, timezone = ?, updated_at = ? WHERE id = ?`,
		user.DisplayName, user.AvatarURL, user.Phone, user.Timezone, now, user.ID.String(),
	)
	if err != nil {
		return err
	}
