...
func (s *PGUserStore) Create(ctx context.Context, user *store.UserData) error {
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO users (id, email, display_name, avatar_url, auth_provider, password_hash, status, phone, timezone, last_login_at, created_at, updated_at)
		...
		sql.NullString{String: derefStr(user.Timezone), Valid: user.Timezone != nil && *user.Timezone != ""},
...
func (s *PGUserStore) GetByID(ctx context.Context, id uuid.UUID) (*store.UserData, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT id, email, display_name, avatar_url, auth_provider, password_hash, status, phone, timezone, last_login_at, created_at, updated_at
...
func (s *PGUserStore) Update(ctx context.Context, user *store.UserData) error {
	_, err := s.db.ExecContext(ctx,
		`UPDATE users SET display_name = $1, avatar_url = $2, phone = $3, timezone = $4, updated_at = NOW() WHERE id = $5`,
		user.DisplayName,
		sql.NullString{String: derefStr(user.AvatarURL), Valid: user.AvatarURL != nil && *user.AvatarURL != ""},
		sql.NullString{String: derefStr(user.Phone), Valid: user.Phone != nil && *user.Phone != ""},
		sql.NullString{String: derefStr(user.Timezone), Valid: user.Timezone != nil && *user.Timezone != ""},
...
func (s *PGUserStore) List(ctx context.Context, tenantID uuid.UUID, params store.UserListParams) (*store.UserListResult, error) {
	...
	query := fmt.Sprintf(
		`SELECT u.id, u.email, u.display_name, u.avatar_url, u.auth_provider, u.password_hash, u.status, u.phone, u.timezone, u.last_login_at, u.created_at, u.updated_at,
	...
	for rows.Next() {
		...
		var avatarURL, passwordHash, phone, timezone sql.NullString
		var lastLoginAt sql.NullTime
		if err := rows.Scan(
			&u.ID, &u.Email, &u.DisplayName, &avatarURL, &u.AuthProvider, &passwordHash,
			&u.Status, &phone, &timezone, &lastLoginAt, &u.CreatedAt, &u.UpdatedAt, &total,
		); err != nil {
			return nil, err
		}
		u.AvatarURL = nilStr(avatarURL.String)
		u.Phone = nilStr(phone.String)
		u.Timezone = nilStr(timezone.String)
...
func scanUserRow(row *sql.Row) (*store.UserData, error) {
	...
	err := row.Scan(
		&u.ID, &u.Email, &u.DisplayName, &avatarURL, &u.AuthProvider, &passwordHash,
		&u.Status, &phone, &timezone, &lastLoginAt, &u.CreatedAt, &u.UpdatedAt,
	)
	if err != nil {
		return nil, err
	}
