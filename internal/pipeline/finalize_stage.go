...
func (s *FinalizeStage) Execute(ctx context.Context, state *RunState) error {
	...
	if s.deps.CheckScopeGuard != nil && state.Observe.FinalContent != "" {
		if onTopic, replacement := s.deps.CheckScopeGuard(state.Input.SessionKey, state.Input.Message, state.Observe.FinalContent, state.Tool.CalledToolNames); !onTopic {
