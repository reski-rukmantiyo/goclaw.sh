...
**Version**: 0.6-draft
**Date**: 2026-09-06
**Status**: Implemented + live-verified on the master tenant (operator confirmed override-contact DM answered by the override agent after the v0.5 factory fix). Contacts editor now its own tab.
...
| 0.6-draft | 2026-09-06 | **Live-verified + UI moved to its own tab (operator-requested).** Operator confirmed the v0.5 factory fix closes the loop: DM from override contact `6281511488487@s.whatsapp.net` (LID-only sender `141089709252847:78@lid`) is answered by the override agent (Kala). FR-02/FR-04/FR-08 live boxes flipped. **UI change:** the "Direct Message Contacts" editor moved out of the Groups tab into a dedicated **Contacts** tab (`channel-contacts-tab.tsx`, WhatsApp-only via `showContactsTab`, `?tab=contacts` deep-link supported, `detail.tabs.contacts` i18n × en/vi/zh; Groups tab returns to join rules + group overrides with its own Save). **Stay-on-tab fix:** `channel-detail-page.tsx` re-resolved the active tab from the URL on every `instance` change — a save refetches the instance, so the effect re-fired and reset the tab to the default after every save. Effect deps changed `instance` → `instance?.id` (identity, stable across refetch), so saving keeps the operator on the Contacts tab (and every other tab — the same reset affected Groups). `pnpm build` ✓, channels vitest 19/19 ✓. |
...
- [x] A DM from a contact with `agent_id: "raka"` produces an agent run under `raka` (verify: session key `BuildScopedSessionKey("raka", <instance>, "direct", <chatID>)` in the `inbound: scheduling message` log, `gateway_consumer_normal.go:194-201`). _(operator live-confirmed 2026-09-06 after the v0.3+v0.4+v0.5 chain: LID resolution → device-strip → factory mapping — DM from `6281511488487@s.whatsapp.net` (LID-only sender) answered by the override agent Kala)_
...
- [x] Operator can add a discovered contact, pick an agent, save, and see the override persist after page reload (config round-trip). _(operator live-confirmed 2026-09-06: UI save → `channel_instances.config` → routing honored — full round-trip proven by the working override; v0.6 moved the editor to its own Contacts tab with its own Save, and saving no longer leaves the tab)_
...
- [x] Live: the operator's DM (`141089709252847:83@lid`, override → `kala`) is answered by Kala after rebuild+restart. _(operator live-confirmed 2026-09-06 — actual sender device was `:78@lid`; resolved to bare phone JID after v0.4 device-strip + v0.5 factory mapping)_
