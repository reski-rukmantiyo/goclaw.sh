...
- **Channel** (`internal/channels/whatsapp/whatsapp.go`): `resolveAgentID` direct branch (FR-02); contact-aware agent-UUID resolution for the listen path (extend `resolveGroupAgentUUID` or sibling); `RefreshGroupAgentCache` also walks `Contacts` (FR-02.1).
...
| LID-only contacts (no `SenderAlt` available) cannot be normalized to a phone JID → lookup misses. | Accepted for v1: normalization requires `SenderAlt` (existing `inbound.go:39-43` behavior for all DM policy checks — pairing/allowlist have the same property). If WhatsApp ever sends LID-only DMs, pairing itself would need the same treatment; not this SRS. |
