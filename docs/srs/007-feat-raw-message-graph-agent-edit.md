...
- [x] Save is disabled when nothing changed or when graph_id is empty/whitespace. _(by inspection — `canSave` gate in `raw-message-detail-dialog.tsx`)_
...
- [x] Submitting with both agent and graph empty is blocked client-side (disabled confirm) and server-side (FR-03). _(client: disabled-confirm gate in `raw-messages-page.tsx`; server: `TestHandleUpdateScope` "neither field provided")_
...
- **Store unit test (SQLite):** same assertions against the SQLite implementation.
- **Handler test:** valid `{ids, agent_id}` / `{ids, graph_id}` / `{ids, both}` → 200 `{updated_count}`; empty `ids` → 400; invalid UUID id → 400; invalid `agent_id` → 400; empty `graph_id` → 400; over-long `graph_id` → 400; both scope fields empty → 400; caller cannot update another tenant's row (`updated_count` excludes it).
...
- **Orphaned prior-scope chunks (embeddings menu).** Resetting `embedded_at` makes the embedding worker write **fresh** chunks under the new `(agent_id, graph_id)`, but the **old** chunks under the prior scope remain in `raw_message_chunks`. The embeddings menu (`GET /v1/embeddings`) reads that table, so until cleanup the message is visible under **both** the old scope (stale chunks) and the new scope (fresh chunks) — additive, not a clean "move". The menu already exposes `POST /v1/embeddings/delete-by-chat` and `POST /v1/embeddings/delete` (`internal/http/embeddings.go:122,95`) for manual cleanup (`delete-by-chat` needs the **old** agent+chat). Automated "true move" is a follow-up — see §8.
...
| Re-embed is additive: the message appears in the embeddings menu under **both** the old scope (stale chunks) and the new scope (fresh chunks), not a clean "move". | Accepted for now (consistent with the KG-entity handling in §7). A true move would require deleting the message's old-scope chunks on scope change — but `RawMessageChunkStore` has no delete-by-source-msg-IDs method (only `DeleteByGraphID`/`DeleteByChatID`/`DeleteByIDs`, `raw_message_chunk_store.go:71-77`), `DeleteByChatID(oldAgent, chat)` would over-delete other messages in the same chat, and wiring the chunk store into the listen handler is cross-store. Defer; operator cleans stale old-scope chunks via the embeddings menu (`delete-by-chat`). If a clean move is required, add `DeleteBySourceMsgIDs` + wire `RawMessageChunkStore` into the listen handler as a follow-up. |
...
8. **Tests:** store (PG + SQLite), handler validation/tenant-scope, frontend edit/batch per §5.
9. **Checklist:** `go build ./...`, `go build -tags sqliteonly ./...`, `go vet ./...`, `pnpm build` in `ui/web`.
10. **Manual verification:** on the master tenant, take a raw message captured under the wrong agent+graph (a `003` RC1+RC2 row), edit both to the configured Felix + `project-dell-ioh`, confirm it returns to `pending`, confirm the extraction worker re-extracts it into the corrected scope (§7 / §5 SQL), **and** confirm the embedding worker writes fresh chunks so the message appears in the embeddings menu (`/t/{tenant}/embeddings`) under the new Felix/`project-dell-ioh` scope. Note the stale old-scope chunks remain until manual cleanup (§7) — the message may temporarily show under both scopes.
