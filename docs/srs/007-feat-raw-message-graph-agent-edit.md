...
| `embedded_at` | unchanged (re-embedding follows re-extraction via existing `ListPendingEmbeddings` path) |
...
- Build the SET clause from the provided fields: start with the reset fields (`processed_at = NULL, extraction_status = 'pending', extraction_error = NULL`); append `agent_id = $n` when `agentID != ""`; append `graph_id = $n` when `graphID != ""`.
...
- **Orphaned prior-scope entities.** Entities already extracted under the **old** `(agent_id, graph_id)` are **not** removed by this feature. After editing + reprocessing, the corrected scope gains fresh entities, but the old scope still contains the stale set. For the `003` RC1+RC2 case (Jarvis/`project-sovereign` → Felix/`project-dell-ioh`), this is the same remediation gap as `003` §5.3 (re-key/delete mis-scoped KG entities + `raw_message_chunks`). A separate cleanup task (operator-run, against a backup, per scope) is the intended path — out of scope here.
...
10. **Manual verification:** on the master tenant, take a raw message captured under the wrong agent+graph (a `003` RC1+RC2 row), edit both to the configured Felix + `project-dell-ioh`, confirm it returns to `pending`, and confirm the worker re-extracts it into the corrected scope (§7 / §5 SQL).
