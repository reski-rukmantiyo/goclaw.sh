...
**Version**: 0.5-draft
**Date**: 2026-09-04
**Status**: Implemented (code-complete + build/vet/test-verified, incl. PG integration tests). Live WhatsApp image round-trip on the master tenant pending (§5 env-gated).
**Difficulty**: Medium
**Estimate**: 2–3 days
...
| 0.5-draft | 2026-09-04 | **Live-run bug fixed + PG integration tests added.** First live run on the master tenant surfaced `"whatsapp media enrich worker: failed to list pending rows … expected 3 arguments, got 2"` — two PG placeholder/argument desyncs in the new store methods that SQLite-only unit tests could not catch: (a) `ListPendingMediaEnrichment` called `scopeClause(ctx, idx+1)` (tenant at `$idx+1`) while emitting `LIMIT $idx` AND never bound `filter.MaxRows` — `$idx` referenced-but-unbound → arg-count mismatch; (b) `MarkMediaEnriched` called `scopeClause(ctx, 3)`, colliding the tenant param with the already-bound `id = $3`. Both fixed (filter args → tenant `$idx` → `LIMIT $idx+1`; `MarkMediaEnriched` tenant `$4`). Added `tests/integration/pglistenmedia/` (own subpackage — the parent integration package has an unrelated pre-existing build breakage, `tenant_provision_test.go:166` stale `AddUser` signature, verified identical on clean tree): 7 tests against real pgvector pg18 incl. migration 000091 apply, fresh/backfill poll, bulk-marked rows staying backfill-eligible, `MarkMediaEnriched` reset, FR-04 gates, legacy `media_refs='null'` shape. All green: builds (PG+sqliteonly), `go vet ./...`, whatsapp 17 enrichment tests, sqlite 21 listen tests, pglistenmedia 7. NOTE for operators: on the already-running gateway, `ensureEnrichActivation` had already persisted `enriched_since` + executed the bulk pass-through mark before the failing poll, so after rebuilding/restarting with this fix, fresh images enrich normally and historical images need `listen.media_analysis.backfill_enabled=true` (per D2). |
