...
export function AgentRoutingTab({ agent, onUpdate }: AgentRoutingTabProps) {
  const { t } = useTranslation("agents");
  const initial = normalizeOpenRouterRouting(
    (agent.openrouter_routing as OpenRouterRoutingConfig) ?? {},
  );
  const [routing, setRouting] = useState<OpenRouterRoutingConfig>(initial);
  const [saving, setSaving] = useState(false);

  const isDirty = useMemo(() => {
    const current = normalizeOpenRouterRouting(routing);
    return (
      JSON.stringify(current) !== JSON.stringify(initial) &&
      !(
        isOpenRouterRoutingEmpty(current) && isOpenRouterRoutingEmpty(initial)
      )
    );
  }, [routing, initial]);

  const update = (patch: Partial<OpenRouterRoutingConfig>) => {
    setRouting((prev) => ({ ...prev, ...patch }));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await onUpdate({
        openrouter_routing: isOpenRouterRoutingEmpty(normalized)
          ? null
          : normalized,
      });
    } catch {
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-3 rounded-lg border p-3 sm:p-4 overflow-hidden">
