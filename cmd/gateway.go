...
func runGateway() {
	...
	if pgStores.Tenants != nil {
		...
		methods.NewTenantsMethods(pgStores.Tenants, pgStores.TenantDBConnections, pgStores.TenantDBManager, pgStores.DB, msgBus, workspace, cfg.Database.TenantDBSSLMode, cfg.Database.PostgresDSN, pgStores.SystemConfigs, tenantAuthLoader, os.Getenv("GOCLAW_ENCRYPTION_KEY"), cfg.Gateway.OwnerIDs).Register(server.Router())
		server.SetTenantsHandler(httpapi.NewTenantsHandler(pgStores.Tenants, pgStores.TenantDBConnections, pgStores.TenantDBManager, pgStores.DB, msgBus, workspace, cfg.Database.TenantDBSSLMode, cfg.Database.PostgresDSN))
